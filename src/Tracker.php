...
/**
 * Holds the current request's distributed-trace id, shared across the log line
 * and any outgoing calls so a chain spanning several services stays joinable.
 *
 * {@see boot()} depends on nothing else in this package and can run from the
 * entry point before the framework is built. It adopts an inbound `X-Request-ID`
 * (web header, or the `X_REQUEST_ID` env var for console / queue callers) when it
 * looks sane, otherwise mints a 32-character hex token, and freezes it into the
 * `TRACKING_REQUEST_UUID` constant.
 */
final class Tracker
{
    ...
    /**
     * Resolve the trace id once and define {@see CONSTANT}. Safe to call
     * repeatedly and from every entry point; the first call wins.
     */
    ...
    /**
     * Header pair to merge into an outgoing request.
     *
